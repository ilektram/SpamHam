...
from config import LOG_LEVEL, RF_RAND_GRID, LR_RAND_GRID, SGD_RAND_GRID, SVM_RAND_GRID, DATA_PATH
...
def read_data_as_df(filepath: str) -> Dict[str, pd.DataFrame]:
    df = pd.read_csv(filepath, index_col=0, header=None, names=['Dataset', 'Text', 'SpamHam'])
    df['is_spam'] = np.where(df['SpamHam'].str.lower().str.strip() == 'spam', 1, 0)
    df.drop(['SpamHam'], inplace=True, axis=1)
    LOG.debug("Missing values per column: {}".format(df.isna().sum()))
...
def clean_text(text: str, lemmatizer: WordNetLemmatizer()=lmtzr) -> List:
    ...
    filtered_sentence = [w.strip().lower() for w in word_tokens if not w in stop_words]
...
def preprocess_data(df):
    ...
    for col in X.columns:
        LOG.info(f"Columns we are left with as model input: {col}: {df[col].describe()}")
    return X, y


def do_crossvalidation(model_name, features, labels, save=False):
    if model_name == "RF":
        classifier = RandomForestClassifier(min_samples_split=3, min_samples_leaf=3,
                                            verbose=0, n_jobs=-1, random_state=7)
        param_grid = RF_RAND_GRID
    elif model_name == "LR":
        classifier = LogisticRegression(verbose=0, n_jobs=-1, random_state=7, max_iter=1000)
        param_grid = LR_RAND_GRID
    elif model_name == "SGD":
        classifier = SGDClassifier(verbose=0, n_jobs=-1, random_state=7, max_iter=1000,
                                   n_iter_no_change=10, early_stopping=True, loss='log')
        param_grid = SGD_RAND_GRID
    elif model_name == "SVM":
        classifier = SVC(verbose=0, random_state=7, max_iter=1000, probability=True)
        param_grid = SVM_RAND_GRID
    else:
        LOG.error("Model {} unrecognized. Please specify one of RF, LR, SGD, SVM!".format(model_name))
        return None
...
def get_feature_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    POS_df = pd.DataFrame(df['n_pos'].to_list()).fillna(0)
    LOG.debug("Got POS_df")
    final_df = pd.concat([df, POS_df], axis=0)
    LOG.debug("Got final_df")
    return final_df


def get_tfidf(df: pd.DataFrame, tfidf_vect: TfidfVectorizer()=tfidf_v):
    df.dropna(subset=['lemmas'], inplace=True)
    LOG.debug(f"We have {len(df)} rows left atm.")
    LOG.debug(f"feature columns: {df.columns}")
    tfidf = tfidf_vect.fit_transform(df['lemmas'])
    joblib.dump(tfidf_vect, 'tfidf_vect.joblib')
    tfidf_df = pd.DataFrame(tfidf.toarray(), columns=tfidf_vect.get_feature_names())
    final_df = pd.concat([df, tfidf_df], axis=0)
    return final_df
